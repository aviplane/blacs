...
class BLACS(object):
    ...
    def __init__(self, application):
        ...
        self.panes[
            "tab_bottom_vertical_splitter"
        ] = self.ui.tab_bottom_vertical_splitter
        ...
        self.front_panel_settings = FrontPanelSettings(
            self.settings_path, self.connection_table
        )
        ...
        for device_name, labscript_device_class_name in list(
            ...
            try:
                ...
                self.settings_dict[device_name]["saved_data"] = (
                    tab_data[device_name]["data"] if device_name in tab_data else {}
                )
                ...
                self.failed_device_settings[device_name] = {
                    "front_panel": self.settings_dict[device_name][
                        "front_panel_settings"
                    ],
                    "save_data": self.settings_dict[device_name]["saved_data"],
                }
        ...
        self.plugins = {}
        plugin_settings = (
            eval(tab_data["BLACS settings"]["plugin_data"])
            if "plugin_data" in tab_data["BLACS settings"]
            else {}
        )
        for module_name, module in plugins.modules.items():
            try:
                ...
                self.plugins[module_name] = module.Plugin(
                    ...
                    if module_name in plugin_settings
                    else {}
                )
            ...
            try:
                if hasattr(plugin, "get_tab_classes"):
                    ...
                    for tab_name, TabClass in plugin.get_tab_classes().items():
                        ...
                        self.settings_dict.setdefault(
                            settings_key, {"tab_name": tab_name}
                        )
                        ...
                        self.settings_dict[settings_key]["saved_data"] = (
                            ...
                            if settings_key in tab_data
                            else {}
                        )

                        self.tablist[settings_key] = TabClass(
                            self.tab_widgets[0], self.settings_dict[settings_key]
                        )
        ...
        self.queue.restore_save_data(
            tab_data["BLACS settings"]["queue_data"]
        )  ### Should this be removed? - 2023/09/06 - AP
                ...
                if "menu_items" in menu_parameters:
                    ...
                    if "icon" in menu_parameters:
                        child = parent.addAction(
                            QIcon(menu_parameters["icon"]), menu_parameters["name"]
                        )
                    else:
        ...
        for module_name, plugin in self.plugins.items():
            try:
                ...
                for notification_class in plugin.get_notification_classes():
                    ...
                    plugin_notifications[
                        notification_class
                    ] = self.notifications.get_instance(notification_class)
        ...
        self.settings = Settings(
            file=self.settings_path, parent=self.ui, page_classes=settings_pages
        )
    ...
    def restore_window(self, tab_data):
        ...
        try:
            ...
            if (
                "window_maximized" in tab_data["BLACS settings"]
                and tab_data["BLACS settings"]["window_maximized"]
            ):
            ...
            logger.warning(
                "Unable to load window and notebook defaults. Exception:" + str(e)
            )
    ...
    def order_tabs(self, tab_data):
        ...
        for tab_name, data in tab_data.items():
            ...
            if bool(data["visible"]) and int(data["notebook"]) in self.tab_widgets:
                self.tab_widgets[int(data["notebook"])].tab_bar.setCurrentIndex(
                    int(data["page"])
                )

    def update_all_tab_settings(self, settings, tab_data):
        for tab_name, tab in self.tablist.items():
            self.settings_dict[tab_name]["front_panel_settings"] = (
                settings[tab_name] if tab_name in settings else {}
            )
            self.settings_dict[tab_name]["saved_data"] = (
                tab_data[tab_name]["data"] if tab_name in tab_data else {}
            )
        ...
        if dialog.exec_():
            ...
            if filepath.endswith(".h5") or filepath.endswith(".hdf5"):
                try:
                    ...
                    logger.exception(
                        "Unable to load the front panel in %s." % (filepath)
                    )
                    message = QMessageBox()
                    message.setText(
...
class RemoteServer(ZMQServer):
    ...
    @inmain_decorator()
    def handle_queued_shots(self):
        n_shots = self.handle_get_n_shots()
        shot_list = [app.queue._model.item(i).text() for i in range(n_shots)]
        return shot_list
...
if __name__ == "__main__":
    if "tracelog" in sys.argv:
        ...
        labscript_utils.tracelog.log(
            os.path.join(BLACS_DIR, "blacs_trace.log"),
            [
                "__main__",
